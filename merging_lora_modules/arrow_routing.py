...
class CustomPeftModel(PeftModel):
    ...
    def forward(self, input_ids, k, *args, **kwargs):
        
        # TODO: Modify the forward pass to iterate on the layers and apply the selected prototype
        for module in self.base_model.modules(): 
            if isinstance(module, LoraLayer): 
                if proj_counter % 2 == 0:
                    ...
                    ABs = {'lora_A': concat_lora_A_dict, 'lora_B': concat_lora_B_dict}

                    vectors_dict = {}
                    eigvals_dict = {}
                    scores = {}
                    # cumpute prototype of experts in the current layer
                    for cluster in ABs['lora_A'].keys():

                        A = ABs['lora_A'][cluster]
                        # A = [tensor.tolist() for tensor in A.values()]

                        B = ABs['lora_B'][cluster]
                        ...
                        vectors_dict[cluster] = top_vector.detach().cpu().numpy()
