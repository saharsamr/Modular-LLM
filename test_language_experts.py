...
from data_handler.dataset import read_dataset, read_LE_dataset, create_message_column_for_test
...
if __name__ == "__main__":
    ...
    test_data = read_LE_dataset(
      '/root/.cache/huggingface/hub/datasets--AliEdalat--LE_dataset_5token_pred/snapshots/f08bedce02865984f127af7e3ba9796c8c51f583/en_Wiki_10k_LM_511_1 (1).json',
      '/root/.cache/huggingface/hub/datasets--AliEdalat--LE_dataset_5token_pred/snapshots/f08bedce02865984f127af7e3ba9796c8c51f583/en_Wiki_10k_LM_511_1_test (1).json', return_test=True)
    ...
    for i, batch in tqdm(enumerate(test_data), total=len(test_data)):
        outputs = pipe(batch['text'], max_new_tokens=8)
        preds = [output['generated_text'].split("<|assistant|>\n")[1].strip() for output in outputs]

        m = len(preds[0].split())
        n = len(batch['target'].split())
        l = min(m,n)

        print([' '.join(batch['target'].split()[:l])])
        print([' '.join(preds[0].split()[:l])])
        
        references.extend([' '.join(batch['target'].split()[:l])])
        predictions.extend([' '.join(preds[0].split()[:l])])
        # break
